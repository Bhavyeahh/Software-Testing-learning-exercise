...
class Game:
    ...
    def _move_and_resolve(self, player, steps):
        ...
        if tile == "go_to_jail":
            ...
            card = self.card_decks["chance"].draw()
            self._apply_card(player, card)

        elif tile == "community_chest":
            card = self.card_decks["community_chest"].draw()
            self._apply_card(player, card)
    ...
    def auction_property(self, prop):
        ...
        if prop.owner is not None:
            print(f"  Auction cancelled: {prop.name} is already owned.")
            return
    ...
    def _apply_card_pay(self, player, value):
        """Collect `value` from player and send it to the bank."""
        player.deduct_money(value)
        self.bank.collect(value)

    def _apply_card_jail(self, player, _value):
        """Send player directly to jail."""
        player.go_to_jail()
        print(f"  {player.name} has been sent to Jail!")

    def _apply_card_jail_free(self, player, _value):
        """Grant player one get-out-of-jail-free card."""
        player.get_out_of_jail_cards += 1
    ...
    def _apply_card(self, player, card):
        ...
        handlers = {
            "collect": self._apply_card_collect,
            "pay": self._apply_card_pay,
            "jail": self._apply_card_jail,
            "jail_free": self._apply_card_jail_free,
            "move_to": self._apply_card_move_to,
            "birthday": self._apply_card_transfer_from_all,
            "collect_from_all": self._apply_card_transfer_from_all,
        }
